...
function getStudentModules($details) {
	...
	/*
	 * Modules(ModuleID, ModuleTitle, Fake, QuestionaireID) StudentsToModules(UserID, QuestionaireID) The goal is to get only modules the student is doing, or where Module.Fake = true An inner join would do the first goal, but not the second so we simulate it by doing a left join (matches all other students) and filter it down using a where to either the student or where fake=true
	 */
...
function getStudentModuleLecturers($details) {
	global $db;
	/*
	 * The goal of this one is to retrieve a list of staff paired against each module, it was originally written to filter down to student but added too much complexity. The join is right, to allow for some tolerance for incomplete data, a missing staff name will simply make their name show as blank.
	 */
	$stmt = new tidy_sql($db, "
