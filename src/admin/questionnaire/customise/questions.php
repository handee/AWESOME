...
function getModule() {
	...
	if (!$moduleID) {
		return array(
			"ModuleID" => "global",
			"ModuleTitle" => "Repeated questions",
			"Fake" => true 
		);
	}
	else {
...
function insertQuestion($details) {
	...
	try {
		...
		$alerts[] = array(
			"type" => "success",
			"message" => "Sucessfully added question" 
		);
	}
	catch (Exception $e) {
		$alerts[] = array(
			"type" => "danger",
			"message" => "Sorry, an error occurred adding question ({$e->getMessage()})" 
		);
	}
}
...
function deleteQuestion($questionID) {
	...
	try {
		...
		$alerts[] = array(
			"type" => "success",
			"message" => "Sucessfully deleted question" 
		);
	}
	catch (Exception $e) {
		$alerts[] = array(
			"type" => "danger",
			"message" => "Sorry, an error occurred deleting question ({$e->getMessage()})" 
		);
	}
}
